...
static void
test_odc_large_inode_synthesis(void)
{
	static const int64_t source_inodes[] = {
		INT64_C(0x40001),
		INT64_C(0x80001)
	};
	static const char *const names[] = {"odc-large-a", "odc-large-b"};
	struct archive_entry *ae;
	struct archive *a;
	int64_t archived_inodes[2];
	char *buff;
	...
	assert((a = archive_write_new()) != NULL);
	assertEqualIntA(a, ARCHIVE_OK, archive_write_set_format_cpio_odc(a));
	assertEqualIntA(a, ARCHIVE_OK, archive_write_add_filter_none(a));
	assertEqualIntA(a, ARCHIVE_OK,
	    archive_write_open_memory(a, buff, buffsize, &used));
	...
	failure("odc synthetic inode values must be in range and distinct");
	assertEqualInt(1, archived_inodes[0]);
	assertEqualInt(2, archived_inodes[1]);
	assert(archived_inodes[0] != archived_inodes[1]);
...
odc_call = "\ttest_format(archive_write_set_format_cpio_odc);\n"
newc_call = "\ttest_format(archive_write_set_format_cpio_newc);\n"
if text.count(odc_call) != 1 or text.count(newc_call) != 1:
    raise RuntimeError("unexpected cpio format test call count")
text = text.replace(
    odc_call,
    odc_call + "\ttest_odc_large_inode_synthesis();\n",
    1,
)
text = text.replace(
    newc_call,
    newc_call + "\ttest_newc_large_inode_collision();\n",
    1,
)

path.write_text(text)
print("cpio-large-inode-baseline-tests-applied")
